...
Selanjutnya, saya membuat `main.js` di dalam folder `TP/main`. Saya jalankan `npm init -y`dahulu baru saya lakukan `npm install`. Jadi library nanti bisa digunakan seperti package pada umumnya. Setelah itu, saya mengimpor fungsi dari library ke dalam file `main.js`.
