...
Kode awal error karena semua data langsung dianggap string, padahal ada data angka dan boolean. Dari sini data perlu diubah dulu menjadi string menggunakan `String(data)`, baru setelah itu bisa memakai `toLowerCase()`. Lalu di sini saya menambahkan `Number(str)` untuk pengecekannya agar data seperti `"123"` dan `456` bisa diproses sebagai angka. tapi kalau tanpa `Number(str)` juga masih normal saja, cukup ganti saja di kecacatan utamanya yaitu :
```
const str = data.toLowerCase();
```
Karena `toLowerCase()` hanya bisa dipakai pada string. Sementara isi array data ada yang berupa number dan boolean, Jadi program akan mengalami error ketika memproses 456 saja.

ubah menjadi:
```
const str = String(data).toLowerCase(); 
```
Maka hasilnya sesuai yang ditampilkan di output `after.png`
